...
#undef ASSERT
#define ASSERT(e) if(!(e)){LCODE__Debug_Printf_Service("Assertion failure in " __FILE__ ", line %d: " #e "rn", __LINE__);
_asm int 1
}
#endif // DBG
#include "NonStubs.h"
...
{ // locked too long
KdPrint(("RemoveLock acquired from file %s, line %d (tag %X) has been locked too longn", t->file, t->line, t->tag));
ASSERT(FALSE);
} // locked too long
if (!found && t->tag == tag)
...
InterlockedIncrement(&p->Dbg.LowMemoryCount);
KdPrint(("RemoveLock never acquired with tag %Xn", tag));
ASSERT(FALSE);
} // should have found it but didn't
} // check debug info
#endif // DEBUG
